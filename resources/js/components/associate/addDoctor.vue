...
<div class="w-full lg:w-2/3 xl:w-2/3 2xl:w-2/3 mx-3 my-3 mx-auto rounded-3xl p-3 bg-white">
 ...
 <form @submit.prevent="claimProfile" enctype="multipart/form-data">
  ...
  <div class="flex">
    <select class="border-2 border-tiruhakim text-tiruhakim text-xl rounded-md px-3 py-1 ml-1 mr-3 my-1">
      ...
      <div>
       ...
       <div v-if="popup" class="absolute top-1/4 left-1/3 bg-white z-20 rounded-xl py-5 px-10">
        ...
        <input type="text" v-model="specialityName" required placeholder="new speciality" :class="[error? 'border-red-500':'border-gray-200']" class="m-1 border-2 rounded-xl w-72 p-2 text-xl outline-none focus:border-b-2 focus:border-b-tiruhakim">
        <button class="bg-tiruhakim text-white text-xl py-2 px-4 rounded-2xl"><span class="bg-white text-tiruhakim w-8 h-8 text-center rounded-full m-1">+</span>add new</button>  
       </div>
  ...
  <div class="my-3">
    ...
    <input type="file" name="file" required class="rounded-lg m-1 text-xl text-tiruhakim border-2 border-tiruhakim"/>
  </div>
